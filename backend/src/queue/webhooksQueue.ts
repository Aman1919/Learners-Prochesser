...
import { updateTransactionChecks } from "../utils";
...
webhookQueue.process(async (job:any) => {
  ...
  // }
    try {
      // Adding all the webhook data to prisma for debugging purpose
      await prisma.webhook.upsert({
        where: {
          apiRef: job?.data?.payload?.api_ref ?? job?.data?.payload?.tracking_id,
        },
        update: {
...
webhookQueue.on("failed", async (job: any, err:any) => {
  console.error(`Job failed: ${job.id}`, err.message);
  await prisma.webhook_retries.create({
    data: {
      status: "FAILURE",
...
webhookQueue.on("completed", async (job:any) => {
  await prisma.webhook_retries.create({
    data: {
      status: "SUCCESS",
