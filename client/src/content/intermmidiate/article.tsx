...
import IntermmidiateArticle from './intermmidiateArticle.json';


const ChessConcepts: React.FC = () => {
  const sectionRefs = IntermmidiateArticle.concepts.map(() => useRef<HTMLDivElement | null>(null));
  ...
  return (
    <div className="flex relative justify-between p-2">
      ...
      <div className="p-6 ml-auto w-full">
        <h1 className="text-3xl font-bold mb-6 text-center">Intermediate Chess Concepts</h1>
        {IntermmidiateArticle.concepts.map((concept, index) => (
...
>
  ✕
            ...
            <div
                className={`absolute top-0 right-0 min-h-[400px] rounded  p-4 transform ${isSidebarOpen ? "translate-x-0 bg-black opacity-90" : "translate-x-full"
                    } transition-transform duration-300 ease-in-out md:static md:translate-x-0`}
            >
                <h2 className="text-xl text-white font-semibold mb-4">Chess Guide Sections</h2>
                <ul>
                    {IntermmidiateArticle.concepts.map((section, index) => (
